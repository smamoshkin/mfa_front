...
### Техдолг: ABC-категоризация и оборачиваемость
...
- **Категория по оборачиваемости** — **заглушка**: на бэке нет данных об остатках товара на складе, поэтому `turnoverDays` всегда `0`, и `getTurnoverCategory` всегда возвращает `C` (худшую категорию, намеренно — чтобы не завышать итоговую оценку).
- **Общая категория** — матрица маржа × оборачиваемость. Из-за заглушки оборачиваемости общая категория **никогда не может быть A** (минимальный ранг = 2 → B), даже у лучших по марже товаров. Это ожидаемое поведение, не баг.
- Когда на бэке появится реальный расчёт оборачиваемости (нужны остатки со склада, которых сейчас нет нигде в проекте — ни в типах, ни в API), достаточно заменить `const turnoverDays = 0` внутри `withAbcCategories` на реальное значение — вся остальная логика (пороги, матрица, отображение) готова и менять не нужно.
...
4. **Оборачиваемость товара** на странице Аналитики — захардкожен `0` (см. раздел "Страница Аналитики" выше), реальных остатков со склада в проекте пока нет.
...
9. **Получить остатки товара со склада WB** (через Statistics API) и передавать реальную оборачиваемость в `/analytics/rentability`, чтобы заменить заглушку `turnoverDays = 0` в ABC-категоризации на странице Аналитики
